...
export default function AdminCheckIns() {
    ...
    const fetchCheckIns = async () => {
        try {
            // Always bypass cache for admin — need fresh data before actions
            const response = await fetch(`/api/checkin?nocache=${Date.now()}`);
                ...
                <div className="flex justify-between items-center mb-8">
                    <div>
                        ...
                        <p className="text-gray-600 mt-1">
                            Manage check-in records — delete or check out sessions
                        </p>
                    </div>
                    <button
                        onClick={() => fetchCheckIns()}
                        className="bg-gray-600 text-white py-2 px-6 rounded-md hover:bg-gray-700 transition-colors font-medium"
                    >
                        Refresh
                    </button>
                </div>
